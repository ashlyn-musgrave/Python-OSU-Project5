...
# ---------------------------------------------------------------------------
    ...
    def build_heap(self, da: DynamicArray) -> None:
        """
        Builds a proper MinHeap from the given DynamicArray in O(N) time complexity.
...
def heapsort(arr: DynamicArray) -> None:
    ...
    def heapify_down(index: int, heap_size: int) -> None:
        """
        Restores the heap property by moving the element at the given index down the heap.
        """
        while True:
            ...
            # Compare with left child
            if left_child_index < heap_size and arr[left_child_index] > arr[largest_child_index]:
                largest_child_index = left_child_index

            # Compare with right child
            if right_child_index < heap_size and arr[right_child_index] > arr[largest_child_index]:
                largest_child_index = right_child_index

            # If the largest child is the current node, the heap property is restored
            if largest_child_index == index:
                break

            # Swap the current node with the largest child
            arr[index], arr[largest_child_index] = arr[largest_child_index], arr[index]

            # Move to the next level in the heap
            index = largest_child_index

    # Build a max heap from the input array using heapify_down
    for i in range((arr.length() // 2) - 1, -1, -1):
        heapify_down(i, arr.length())

    # Perform Heapsort by repeatedly extracting the maximum element
